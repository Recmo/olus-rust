...
pub fn parse_file(name: &str) -> AST::Statement {
    let file = File::open(name).expect("Can not open file.");
    let mmap = unsafe { Mmap::map(&file).expect("Can not mmap file.") };
    let text = std::str::from_utf8(mmap.as_ref()).expect("File not UTF8.");
